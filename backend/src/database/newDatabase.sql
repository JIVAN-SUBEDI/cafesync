...
CREATE TABLE hotels (
    ...
    subscription_status VARCHAR(20) DEFAULT 'trial', -- trial, active, suspended, cancelled, trial_expired
    subscription_start_date DATE,
    subscription_end_date DATE,
    trial_ends_at DATE,
    
    -- Registration Type
    registration_type VARCHAR(20) DEFAULT 'subscription', -- 'trial' or 'subscription'
...
CREATE INDEX idx_hotels_subscription_plan_id ON hotels(subscription_plan_id);
CREATE INDEX idx_hotels_created_at_month ON hotels (date_trunc('month', created_at));
CREATE INDEX idx_hotels_is_active_created_at ON hotels(is_active, created_at DESC);
CREATE INDEX idx_hotels_subscription_status_plan ON hotels(subscription_status, subscription_plan_id);
CREATE INDEX idx_hotels_registration_type ON hotels(registration_type);
...
CREATE INDEX idx_staff_last_login ON staff(last_login DESC);
CREATE INDEX idx_staff_hotel_active ON staff(hotel_id, is_active);
CREATE INDEX idx_staff_hotel_created_at ON staff(hotel_id, created_at DESC);

-- Partial unique index for pin_code
-- CREATE UNIQUE INDEX unique_staff_pin_per_hotel
...
CREATE INDEX idx_hotel_tables_hotel_status ON hotel_tables(hotel_id, status);
CREATE INDEX idx_hotel_tables_status_hotel ON hotel_tables(status, hotel_id);
...
CREATE INDEX idx_orders_hotel_status_created_at ON orders(hotel_id, status, created_at DESC);
CREATE INDEX idx_orders_waiter_created_at ON orders(waiter_id, created_at DESC);

-- Partial index for completed orders
CREATE INDEX idx_orders_completed_created_at ON orders(created_at DESC) WHERE status = 'completed';
CREATE INDEX idx_orders_completed_hotel_created_at ON orders(hotel_id, created_at DESC) WHERE status = 'completed';
CREATE INDEX idx_orders_completed_created_date ON orders((DATE(created_at))) WHERE status = 'completed';
CREATE INDEX idx_orders_completed_month ON orders (date_trunc('month', created_at)) WHERE status = 'completed';
...
-- Indexes
CREATE INDEX idx_inventory_categories_hotel_id ON inventory_categories(hotel_id);
...
CREATE TABLE inventory (
    ...
    -- Status & Tracking
    status VARCHAR(20) DEFAULT 'in_stock', -- in_stock, low_stock, out_of_stock, over_stock, discontinued
    is_active BOOLEAN DEFAULT true,
...
CREATE INDEX idx_inventory_active_status ON inventory(is_active, status);
CREATE INDEX idx_inventory_hotel_active_status ON inventory(hotel_id, is_active, status);
...
-- Indexes
CREATE INDEX idx_inventory_transactions_hotel_id ON inventory_transactions(hotel_id);
...
-- Indexes
CREATE INDEX idx_inventory_alerts_hotel_id ON inventory_alerts(hotel_id);
...
-- Indexes
CREATE INDEX idx_menu_item_inventory_hotel_id ON menu_item_inventory(hotel_id);
CREATE INDEX idx_menu_item_inventory_menu_item_id ON menu_item_inventory(menu_item_id);
CREATE INDEX idx_menu_item_inventory_inventory_id ON menu_item_inventory(inventory_id);
...
CREATE TABLE user_terms_acceptance (
    ...
    user_type VARCHAR(50) NOT NULL, -- 'hotel_admin', 'staff', 'customer'
    hotel_id UUID REFERENCES hotels(id) ON DELETE CASCADE,
...
CREATE TABLE activity_logs (
    ...
    -- User Info
    user_id UUID,
    user_type VARCHAR(20), -- 'hotel_admin', 'staff', 'main_admin', 'guest'
...
CREATE INDEX idx_activity_logs_user_type_created_at ON activity_logs(user_type, created_at DESC);
CREATE INDEX idx_activity_logs_resource_type_created_at ON activity_logs(resource_type, created_at DESC);
CREATE INDEX idx_activity_logs_resource_type_resource_id ON activity_logs(resource_type, resource_id);
CREATE INDEX idx_activity_logs_session_id ON activity_logs(session_id);

-- Partial indexes for common queries
CREATE INDEX idx_activity_logs_main_admin_recent ON activity_logs(created_at DESC) WHERE user_type = 'main_admin';
CREATE INDEX idx_activity_logs_resource_recent ON activity_logs(resource_type, created_at DESC) 
...
BEGIN
    ...
    SELECT COALESCE(
        MAX(
            CAST(SPLIT_PART(item_code, '-', 2) AS INT)
        ),
        0
    ) + 1
    INTO next_number
...
SELECT 
    ...
    (SELECT COUNT(*) FROM orders o 
     WHERE o.hotel_id = h.id 
     AND o.status IN ('pending', 'confirmed', 'preparing')) as pending_orders
FROM hotels h
...
SELECT 
    ...
    (SELECT COALESCE(SUM(oi.quantity), 0) FROM order_items oi 
     WHERE oi.menu_item_id = mi.id 
     AND DATE(oi.created_at) = CURRENT_DATE) as today_quantity
FROM menu_items mi
...
INSERT INTO subscription_plans (id, plan_name, plan_code, price_per_year, max_staff, max_tables, max_menu_items, description, features, display_order) VALUES
(
    uuid_generate_v4(),
    'Starter',
    'STARTER',
...
CREATE TABLE IF NOT EXISTS pending_registrations (
    ...
    -- admin info (before actual hotel/admin is created)
    admin_name VARCHAR(255) NOT NULL,
